...
int main(int argc, char **argv)
{
  ...
  if (sw_incnt + sw_inrep > 1)
    error("only one of -C and -R can be specified");
  if (sw_incnt + sw_inrep == 0)
  { /* rmt mode */
    fname_rmt = fname_in;
...
int read_assoc()
{
  ...
  if (fname_ass)
  {
    ...
    printf("\n# reading %s", cbuf);
    cm0 = fread_i(fp);
    assvec0 = NEW_A(cm0, int *);
    asslen0 = NEW_A(cm0, int);
    cassvec0 = NEW_A(cm0, int *);
    for (i = 0; i < cm0; i++)
    {
      asslen0[i] = 0;
      assvec0[i] = fread_ivec(fp, &(asslen0[i]));
  ...
  for (i = 0; i < cm0; i++)
    cassvec0[i] = getcass(assvec0[i], asslen0[i], mm, NULL);
  printf("\n# CM:%d", cm0);
  if (cm <= 0 || cm > cm0)
    cm = cm0;
  if (cm < cm0)
  ...
  for (i = 0; i < cm0; i++)
  {
    ...
    for (j = 0; j < dvbuf[i]->len; j++)
      mydprintf(1, " %6.2f", dvbuf[i]->ve[j]);
  }
  ...
  for (i = 0; i < cm; i++)
  {
    ...
    for (j = 0; j < asslen[i]; j++)
      assvec[i][j] = assvec0[orderv[i]][j];
    cassvec[i] = NEW_A(mm - asslen[i], int);
    for (j = 0; j < mm - asslen[i]; j++)
      cassvec[i][j] = cassvec0[orderv[i]][j];
  }
...
int do_rmtmode()
{
  ...
  { /* binary read from file */
    ...
    i = fread_bi(fp);
    if (i != kk)
      error("wrong size in rmt");
    repmats = NEW_A(kk, double **);
    for (i = 0; i < kk; i++)
    {
      repmats[i] = fread_blmat(fp, &mm, &(bb[i]));
      putdot();
    }
    fclose(fp);
    FREE(cbuf);
  ...
  printf("\n# R:");
  for (i = 0; i < kk; i++)
    printf("%g ", rr[i]); // scales
  printf("\n# B:");
  for (i = 0; i < kk; i++)
    printf("%d ", bb[i]); // no of reps
...
int congcnt(double ***cntmatm, int **bbm, int mf)
{
  ...
  for (j = 0; j < kk; j++)
  {
    ...
    if (sw_cong)
      vamat[cm][j] = z - y * fsquare(pvmat[cm][j]);
  }
...
int do_rmtmultimode()
{
  ...
  for (f = 0; f < mf; f++)
  {
    ...
    for (i = 0; i < kk; i++)
      printf("%6d ", bbm[f][i]);
    for (i = 0; i < kk; i++)
      if (fabs(rrm[f][i] - rrm[0][i]) > rrm[0][i] * 0.1)
        error("rr mismatch");
    i = fread_bi(fp);
    if (i != kk)
      error("wrong size in rmt");
...
int do_repmode()
{
  ...
  { /* binary read from file */
    ...
    printf("\n# reading %s", cbuf);
    orderv = fread_bivec(fp, &cm);
    obsvec = fread_bvec(fp, &cm);
    rr = fread_bvec(fp, &kk);
    bb = fread_bivec(fp, &kk);
    i = fread_bi(fp);
    if (i != kk)
      error("wrong number of matrices");
    statmats = NEW_A(kk, double **);
    for (i = 0; i < kk; i++)
      statmats[i] = fread_blmat(fp, &cm, bb + i);
    fclose(fp);
    FREE(cbuf);
  }
  else
...
int do_cntmode()
{
  ...
  kk = cm = 0;
  orderv = fread_ivec(fp, &cm);
  obsvec = fread_vec(fp, &cm);
  rrmat = fread_mat(fp, &cm, &kk);
  bbmat = fread_mat(fp, &cm, &kk);
...
int do_bptest()
{
  ...
  for (j = 0; j < cm; j++)
    npvec[j] = 1.0 - pvaldist(statmats[i0][j], nb, threshold);
...
int do_batest()
{
  ...
  if (npdm != mm)
  {
    ...
    for (i = 0; i < mm; i++)
    {
      y += buf1[i] = exp(bapcoef * (datvec[i] - x));
    }
    for (i = 0; i < mm; i++)
      buf1[i] = buf1[i] / y; // normalization
  }
  else
  {
    ...
    printf("\n# ppn:%g", bapn);
    for (i = 0; i < mm; i++)
      buf1[i] = bapcoef * datvec00[i] - 0.5 * pdmvec[i] * log(bapn);
    x = -HUGENUM;
    for (i = 0; i < mm; i++)
      if (buf1[i] > x)
        x = buf1[i];
...
int do_bootrep()
{
  ...
  /* memory alloc for ci */
  cimat = new_mat(cm, nalpha);
  csmat = new_mat(cm, nalpha);
  eimat = new_mat(cm, nalpha);
  ci0mat = new_mat(cm, nalpha);
  cs0mat = new_mat(cm, nalpha);
  ...
  if (sw_fitmode == FITMODE_CHI)
    sw_fitmode = FITMODE_MLE;
  for (j = 0; j < cm; j++)
  {
    for (i = 0; i < kk; i++)
      statp[i] = statmats[i][j];
    for (i = 0; i < nalpha; i++)
    {
      ...
      if (k)
        printf(" au item=%2d, alpha=%4.2f",
               j + 1, alphavec[i]);
      k = invtpval(statp, rr, bb, kk, alphavec[i],
                   &(ci0mat[j][i]), &(cs0mat[j][i]), &(ei0mat[j][i]), -1.0);
      if (k)
        printf(" bp item=%2d, alpha=%4.2f",
               j + 1, alphavec[i]);
    }
    putdot();
  }
...
int write_pv(int sw_bp, int sw_ba, int sw_mc, int sw_au)
{
  ...
  outbit = (sw_bp ? BPPVSW : 0) + (sw_ba ? BAPVSW : 0) + (sw_mc ? MCPVSW : 0) + (sw_au ? AUPVSW : 0);
  pvnum = (sw_bp ? BPPVNUM : 0) + (sw_ba ? BAPVNUM : 0) + (sw_mc ? MCPVNUM : 0) + (sw_au ? AUPVNUM : 0);
  ...
  for (i = 0; i < cm; i++)
  {
    ...
    {
      auxmat[i][j] = pfvec[i];
      j++; /* pvalue of diagnostic */
      auxmat[i][j] = rssvec[i];
      j++; /* rss */
      auxmat[i][j] = dfvec[i];
      j++; /* df */
      auxmat[i][j] = betamat[i][0];
      j++; /* signed distance */
      auxmat[i][j] = betamat[i][1];
      j++; /* curvature */
      auxmat[i][j] = thvec[i];
      j++; /* threshold */
      auxmat[i][j] = betamat[i][2];
      j++; /* dim */
    }
  }
...
int write_rep()
{
  ...
  { /* binary write to file */
    ...
    printf("\n# writing %s", cbuf);
    fwrite_bivec(fp, orderv, cm);
    fwrite_bvec(fp, obsvec, cm);
    fwrite_bvec(fp, rr, kk);
    fwrite_bivec(fp, bb, kk);
    fwrite_bi(fp, kk);
    for (i = 0; i < kk; i++)
    {
      fwrite_bmat(fp, statmats[i], cm, bb[i]);
      putdot();
    }
    fclose(fp);
    FREE(cbuf);
  ...
  { /* ascii write to stdout */
    printf("\n# writing to stdout\n");
    write_ivec(orderv, cm);
    write_vec(obsvec, cm);
    write_vec(rr, kk);
    write_ivec(bb, kk);
    write_i(kk);
...
double cntdist3(double *vec, int bb, double t)
{
  ...
  {
    if (vec[i + 1] > vec[i])
      p = 0.5 + (double)i + (t - vec[i]) / (vec[i + 1] - vec[i]);
    else
      p = 0.5 + (double)i; /* <- should never happen */
  }
  else
  {
    if (vec[bb - 1] - vec[bb - 2] > 0)
      p = n - 0.5 + (t - vec[bb - 1]) / (vec[bb - 1] - vec[bb - 2]);
    else
      p = n;
  }
  if (p > n)
...
double cntdist(double *vec, int bb, double t, int modesw)
{
  ...
  {
  case 1:
    return (double)cntdist1(vec, bb, t);
  case 2:
    return (double)cntdist2(vec, bb, t);
  case 3:
...
double pvaldist(double *vec, int bb, double t)
{
  ...
  if (sw_pvaldist)
    x = cntdist(vec, bb, t, 4); /* smoothing */
  else
    x = cntdist(vec, bb, t, 1); /* nonsmoothing */
                  ...
                  int *ass, int *cass, int alen, double **wt)
{
  ...
  for (i = 0; i < alen; i++)
    pvec[i] = pvaldist(pmat[i], nb, stat[i]);
                  ...
                  int *ass, int *cass, int alen, double **wt)
{
  ...
  if (wt)
  {
    ...
    for (i = 0; i < alen; i++)
    {
      ...
      {
        ja = cass[j];
        y = wt[ia][ja] * (datvec[ja] - datvec[ia]);
        if (y > x)
        {
          x = y;
          jj = j;
  ...
  ja = cass[j0];
  ia = ass[i0];
  x0 = datvec[ja] - datvec[ia];
  for (i = 0; i < nb; i++)
    repvec[i] = repmat[ja][i] - repmat[ia][i];
...
double *getseval(double *pval, int m, double nb, double *seval)
{
  ...
  if (seval == NULL)
    seval = new_vec(m);
  for (i = 0; i < m; i++)
    seval[i] = sqrt(pval[i] * (1.0 - pval[i]) / nb);
                ...
                double rmin, double rmax, double kappa)
{
  ...
  *df = (double)(m - 2);
  ...
  else
  {
    ...
    *se = sqrt(x * x * (vmat[0][0] + kappa * kappa * vmat[1][1] - kappa * vmat[0][1] - kappa * vmat[1][0]));
            ...
            double rmin, double rmax)
{
  ...
  for (loop = 0; loop < mleloopmax; loop++)
  {
    ...
    update[0] = v11 * d1f + v12 * d2f;
    update[1] = v12 * d1f + v22 * d2f;
    coef[0] += update[0];
    coef[1] += update[1];

    /* check convergence */
    e = -d11f * update[0] * update[0] - 2.0 * d12f * update[0] * update[1] - d22f * update[1] * update[1];
  ...
  *df = 0.0;
  for (i = 0; i < m; i++)
  {
    if (p[i] > 0.0 && p[i] < 1.0)
    {
      *df += 1.0;
      if (c[i] > 0.0)
        a = c[i] * log(c[i] / b[i] / p[i]);
      else
        a = 0.0;
      if (c[i] < b[i])
        a += (b[i] - c[i]) * (log3(p[i]) - log3(c[i] / b[i]));
      *lrt += a;
    }
  }
  *lrt *= 2.0;
  *df -= 2.0;
  ...
  coef0[1] = coef[1];
  vmat[0][0] = v11;
  vmat[0][1] = vmat[1][0] = v12;
  vmat[1][1] = v22;
  if (loop == mleloopmax || *df < -0.01)
    i = 1;
  else
    i = 0;
  return i;
 ...
 spherical region. m is the dimension of the space */
{
  ...
  for (k = 0; k < kk; k++)
  {
    ...
    {
      if (!aneg)
        bp = pchisqnc(aa * rr[k], m, xx * rr[k]);
      else
        bp = tchisqnc(aa * rr[k], m, xx * rr[k]);
    }
    else
    {
      r = sqrt(rr[k]);
    ...
    {
      if (cc[k] > 0.0)
        lik = cc[k] * log(cc[k] / bb[k] / bp);
      else
        lik = 0.0;
      if (cc[k] < bb[k])
        lik += (bb[k] - cc[k]) * (log3(bp) - log3(cc[k] / bb[k]));
      lrt += lik;
    }
  }
  lrt *= 2.0;
 ...
 spherical region. m is the dimension of the space */
{
  ...
  {
    if (!aneg)
      pval = tchisqnc(xx, m, aa);
    else
      pval = pchisqnc(xx, m, aa);
  }
  else
  {
            ...
            double rmin, double rmax)
{
  ...
  if (kk > kk0)
  {
    cc0 = renew_vec(cc0, kk);
    rr0 = renew_vec(rr0, kk);
    bb0 = renew_vec(bb0, kk);
    kk0 = kk;
  }
                ...
                double rmin, double rmax, double kappa)
{
  ...
  *pv = chiobj(parm);
  ...
  x = 0.0;
  for (i = 0; i < CHIFUNCP; i++)
    for (j = 0; j < CHIFUNCP; j++)
      x += diff[i] * diff[j] * vmat[i][j];
  if (x < 0.0)
  {
    warning("negative variance of au in chicalcpval");
    x = 0.0;
  }
  *se = sqrt(x);
  ...
  *pv0 = chiobj0(parm);
  ...
  if (x < 0.0)
  {
    warning("negative variance of np in chicalcpval");
    x = 0.0;
  }
  *se0 = sqrt(x);
  ...
  *pvp = *sep = *pv0p = *se0p = *rssp = *dfp = *thp = 0.0;
  ...
  for (j = 0; j < vcloopmax; j++)
  {
        ...
        (idf0 >= dfmin && (z - z0) * (x - *thp) > 0.0 && fabs(z - z0) > vceps2 * ze0))
    {
      mydprintf(1, "\n# non-monotone");
      th = x;
      xn = vcscale * x + (1.0 - vcscale) * (*thp);
      continue;
    }
    if (idf0 >= dfmin && (fabs(z - z0) < vceps1 * ze0))
    {
      if (fabs(th - threshold) < 1e-10)
        xn = th;
      else
        th = x;
    }
    else
      xn = vcscale * th + (1.0 - vcscale) * x;
    *pvp = pv;
    *sep = se;
    *rssp = rss;
    *dfp = df;
